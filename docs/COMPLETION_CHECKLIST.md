...
| **Fivetran SurveyMonkey Ingestion** | **Completed and End-to-End Live Verified** | Connector `prohibited_every` was enabled and active on 5 September 2026. The three latest scheduled incremental runs were successful, including the 1:26:31 PM run, which loaded 7 rows in 21 seconds. Snowflake then showed 58 rows across the 10 SurveyMonkey landing tables, an increase of exactly 7 from the historical 51-row baseline. |
...
3. **Fivetran Historical vs Current:** Fivetran successfully synced 51 rows historically on 31 August 2026. On 5 September, the connector remained active, its latest incremental run loaded 7 rows, and Snowflake showed 58 total rows across the 10 landing tables. This reconciles exactly to the 51 plus 7 expected result.
