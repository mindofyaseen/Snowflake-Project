...
| **Fivetran SurveyMonkey Ingestion** | **Completed, Current Recheck Requires Login** | Connector `prohibited_every` was enabled and active with successful runs on 3 September 2026; latest observed run loaded 7 rows. The 5 September browser session has expired and requires sign-in for a fresh run. |
...
3. **Fivetran Historical vs Current:** Fivetran successfully synced 51 rows historically on 31 August 2026 and was observed enabled and active with successful 7-row runs on 3 September. A fresh 5 September run requires signing back in to the expired browser session.
